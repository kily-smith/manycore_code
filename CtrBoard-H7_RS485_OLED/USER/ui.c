...
static uint32_t g_last_center_press_ms = 0U;
...
static void UI_OnKeyEvent(uint32_t now_ms, ButtonAD_Key_t key)
{
    ...
    {
        if ((g_last_center_press_ms != 0U) && ((now_ms - g_last_center_press_ms) <= UI_DOUBLE_CLICK_MS))
        {
            EmmMotor_StopMove();
            g_last_center_press_ms = 0U;
        }
        else
        {
            EmmMotor_StartMoveToSetting();
            g_last_center_press_ms = now_ms;
        }
        g_ui_need_redraw = 1U;
    }
}
